...
3. **边界可替换**：先用内存适配器（`catga_core::memory`，内置于 catga-core）写应用代码，需要持久化/分布式时只替换对应边界（如用 NATS 替换 `MemoryTransport`），应用模型不变。
...
| 需求 | 依赖 |
| --- | --- |
| 进程内 typed 请求/命令/事件（必需核心） | `catga-core = "0.2"` |
| 补偿性 / 持久化工作流（`DslFlow`/`FlowRuntime`，`catga_core::flow`） | `catga-core = "0.2"` |
| 有界内存适配器、确定性测试（`catga_core::memory`） | `catga-core = "0.2"` |
| SQL/Redis 持久化 Flow 状态 | `catga-flow-store = { version = "0.2", features = ["sqlite"] }` |
| NATS 传输与 JetStream 存储 | `catga-nats = "0.2"` |
...
| 测试（spy/harness/断言） | `catga_core::testing`（catga-core 内置，见 [production.md](production.md)） |
...
本仓库自带无需 Docker 即可运行的示例，源码见 [`examples/`](../../../examples/) 目录。`catga-examples` 包提供三个进程内二进制，写代码前可参考：

```bash
cargo run -p catga-examples --bin quickstart   # 最小 mediator（#[catga_request] + #[catga_service] 宏）
cargo run -p catga-examples --bin order        # 订单服务：CQRS + 事件溯源 + 补偿 Flow 最佳实践
cargo run -p catga-examples --bin doc_demo     # #[catga_service] 自动生成的处理器文档演示
```

分布式集群示例为 [`examples/distributed-kv/`](../../../examples/distributed-kv/)：三节点 Raft 复制的 KV 存储，共识后端可用 `--backend raft|sorock` 切换（raft = catga-cluster + catga-axum HTTP；sorock = catga-sorock gRPC multi-Raft）：
