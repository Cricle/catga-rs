...
`catga-core` 的 `flow` 模块提供三种执行模型，**按持久化与等待需求选型**：
...
- `compensating_flow!` 宏（在 `catga_core` crate 根导出）让「动作 → 补偿」更易读：
...
每个步骤就是一个普通闭包 `Fn(&mut S) -> BoxFuture<CatgaResult<()>>`——直接写 `Box::pin(async move { .. })`，无需任何辅助宏：
...
- 共享并发预算：`FlowThrottle::new(limit)?`（`catga_core::flow::flow_throttle::FlowThrottle`）+ `.throttle(throttle, action)`；分支上限 `MAX_DSL_PARALLEL_BRANCHES`。
...
步骤有**稳定名字**，处理器接收当前 `FlowState` 并返回 `FlowStepOutcome`：
...
// 可回滚步骤：.step_with_compensation("charge", handler, compensation)
```
