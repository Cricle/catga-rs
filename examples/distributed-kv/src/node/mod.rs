...
pub(crate) async fn bench(
    ...
    if batch == 1 {
        println!(
            "writes={writes} total={total_time:.2?} throughput={throughput:.0}/s mean={mean:.2?} p50={:.2?} p99={:.2?}",
            pick(0.50),
            pick(0.99)
        );
    } else {
        println!(
            "writes={writes} keys_per_req={batch} total={total_time:.2?} throughput={throughput:.0} keys/s mean={mean:.2?} p50={:.2?} p99={:.2?}",
            pick(0.50),
            pick(0.99)
        );
    }
...
async fn bench_http(
    ...
    concurrency: usize,
    batch: usize,
) -> CatgaResult<Vec<std::time::Duration>> {
    ...
    if concurrency <= 1 {
        for request in 0..total.div_ceil(batch) {
            ...
            let (url, body) = if batch == 1 {
                (
                    single_url.as_str(),
                    format!(r#"{{"key":"bench-{start}","value":"v{start}"}}"#),
                )
            } else {
                let items: Vec<String> = (start..end)
                    .map(|index| {
                        format!(
                            r#"{{"key":"bench-{request}-{}","value":"v{index}"}}"#,
                            index - start
                        )
                    })
                    .collect();
            ...
            if !response.status().is_success() {
                return Err(CatgaError::new(
                    ...
                    format!(
                        "bench request {request} failed with HTTP {}",
                        response.status()
                    ),
                ));
            }
...
async fn bench_grpc(
    ...
    fn rpc_error(context: impl std::fmt::Display, error: impl std::fmt::Display) -> CatgaError {
        CatgaError::new(
            ErrorCode::Unavailable,
            format!("bench grpc {context}: {error}"),
        )
    }
    ...
    async fn put_batch_with_retry(
        client: &mut KvClient<tonic::transport::Channel>,
        items: PutBatchRequest,
    ) -> Result<(), tonic::Status> {
        let mut attempt = 0u32;
        loop {
    ...
    if concurrency <= 1 {
        let mut client = KvClient::connect(endpoint)
            .await
            .map_err(|error| rpc_error(format_args!("connect to {addr}"), error))?;
        for request in 0..total.div_ceil(batch) {
            let start = request * batch;
            let end = (start + batch).min(total);
            let one = std::time::Instant::now();
            if batch == 1 {
                put_with_retry(
                    &mut client,
                    PutRequest {
                        key: format!("bench-{start}"),
                        value: format!("v{start}"),
                    },
                )
                .await
                ...
                let items: Vec<PutRequest> = (start..end)
                    .map(|index| PutRequest {
                        key: format!("bench-{request}-{}", index - start),
            ...
            handles.push(tokio::spawn(async move {
                ...
                while index < start_index + count {
                    ...
                    if batch == 1 {
                        ...
                        .map_err(|error| {
                            rpc_error(
                                format_args!("request {request} (keys {index}..{end})"),
                                error,
                            )
                        })?;
                    } else {
                        let items: Vec<PutRequest> = (index..end)
                            .map(|key_index| PutRequest {
                                key: format!("bench-{task}-{request}-{}", key_index - index),
                                value: format!("v{key_index}"),
                            })
                            .collect();
                        put_batch_with_retry(&mut client, PutBatchRequest { items })
                            .await
...
pub(crate) async fn run(args: Args, k8s: Option<K8sTopology>) -> CatgaResult<()> {
    ...
    let state = Arc::new(SharedState::new(
        std::path::Path::new(&data_dir).join("kv.redb"),
    )?);
