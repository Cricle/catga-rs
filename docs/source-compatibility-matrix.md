...
| External job schedulers (6 C# files) | `FlowDueService`, `DueFlowScheduler`, Memory/SQL/Redis/NATS schedulers | `tests/flow`, `crates/catga-flow-store/tests/{sqlite,mssql}.rs`, `tests/redis.rs`, `tests/nats.rs` | Replaced with lease-aware pure-Rust scheduling. `SqlFlowScheduler` provides feature-gated SQLite, MySQL, PostgreSQL, and SQL Server persistence with target uniqueness, indexed bounded lease claims, and no worker. The NATS scheduler uses JetStream KV CAS, generation-fenced schedule identities, and a 32-entry paged discovery bound; `FlowDueService` remains the caller-owned poller. |
