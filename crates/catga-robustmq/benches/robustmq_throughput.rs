...
//! RobustMQ transport benchmarks.
//!
//! These benchmarks measure serialization overhead that can be measured without a RobustMQ server.
//! Run with: `cargo bench -p catga-robustmq --bench robustmq_throughput`
...
fn envelope_decoding(c: &mut Criterion) {
    ...
    for (size, encoded) in test_cases {
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, _size| {
            b.iter(|| {
                let decoded: Envelope = codec.decode(black_box(&encoded)).unwrap();
                black_box(decoded)
            });
        });
    }
...

    {
        ...
        for (size, payload) in &test_cases {
            ...
            encode_group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, _size| {
                b.iter(|| {
                    let result = codec.encode(black_box(&envelope)).unwrap();
                    black_box(result)
                });
            });
        }
...

    {
        ...
        for (size, encoded) in &encoded_cases {
            ...
            decode_group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, _size| {
                b.iter(|| {
                    let result: Envelope = codec.decode(black_box(encoded)).unwrap();
                    black_box(result)
                });
            });
        }
...
fn priority_queue_operations(c: &mut Criterion) {
    let codec = MemoryPackCodec::default();

    let priorities: Vec<(&str, QualityOfService)> = vec![
        ("critical", QualityOfService::AtLeastOnce),
        ("high", QualityOfService::AtLeastOnce),
        ("normal", QualityOfService::AtLeastOnce),
        ("low", QualityOfService::AtLeastOnce),
    ];

    let mut group = c.benchmark_group("priority_operations");

    for (name, qos) in priorities {
        let metadata = MessageMetadata::new(1, None).with_quality_of_service(qos);
        let envelope = Envelope::new(1, "test.message", vec![0u8; 256], metadata);
