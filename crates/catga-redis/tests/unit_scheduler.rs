...
fn unix_millis(value: SystemTime) -> Result<i64, String> {
    ...
    i64::try_from(elapsed.as_millis()).map_err(|_| "due time exceeds range".to_string())
}

fn duration_millis(value: Duration) -> Result<i64, String> {
    i64::try_from(value.as_millis()).map_err(|_| "lease duration exceeds range".to_string())
}

fn from_unix_millis(value: &str) -> Result<SystemTime, String> {
    let millis = value
        .parse::<u64>()
        .map_err(|_| "invalid due time".to_string())?;
    UNIX_EPOCH
...
fn target_key_format() {
    ...
    assert!(result.is_ok());
    let key = result.expect("test value must be present");
    assert!(key.len() > 16);
...
fn target_key_empty_ids() {
    ...
    let state_len = u64::from_be_bytes(key[8..16].try_into().expect("test value must be present"));
...
fn target_key_preserves_content() {
    ...
    let key = result.expect("test value must be present");
    let flow_len =
        u64::from_be_bytes(key[0..8].try_into().expect("test value must be present")) as usize;
    let extracted_flow =
        String::from_utf8(key[8..8 + flow_len].to_vec()).expect("test value must be present");
...
fn duration_millis_valid() {
    ...
    assert!(result.is_ok());
    assert_eq!(result.expect("test value must be present"), 500);
}
...
fn duration_millis_seconds() {
    ...
    assert!(result.is_ok());
    assert_eq!(result.expect("test value must be present"), 5000);
}
...
fn from_unix_millis_invalid_string() {
    ...
    assert!(result.is_err());
    assert!(result.expect_err("expected an error").contains("invalid"));
}
...
fn from_unix_millis_large_value() {
    // 8e14 ms (~year 27,000) is representable on both Unix and Windows; u64::MAX
    // milliseconds overflows the Windows SystemTime range during conversion.
    let result = from_unix_millis("800000000000000");
    assert!(result.is_ok());
}
