...
fn unix_millis(time: SystemTime) -> Result<i64, &'static str> {
    match time.duration_since(UNIX_EPOCH) {
        Ok(duration) => i64::try_from(duration.as_millis()).map_err(|_| "time exceeds i64 range"),
...
fn mutation_batch_reasonable() {
    const { assert!(MUTATION_BATCH > 0) };
    const { assert!(MUTATION_BATCH <= 1024) };
}
...
fn version_member_roundtrip_positive() {
    ...
    let parsed = parse_version_member(member.as_bytes()).expect("test value must be present");
...
fn from_unix_millis_roundtrip() {
    ...
    let millis = unix_millis(original).expect("test value must be present");
    let restored = from_unix_millis(millis).expect("test value must be present");
...
fn time_encoding_is_symmetric() {
    ...
    for t in times {
        let millis = unix_millis(t).expect("test value must be present");
        let restored = from_unix_millis(millis).expect("test value must be present");
        let diff = t
            .duration_since(restored)
            .expect("test value must be present");
