...
pub fn expand_impl_handlers(
    ...
    let doctest_module = if !requests.is_empty() {
        let test_fns: Vec<TokenStream> = requests.iter().map(|(_, m)| {
            let method_name = &m.method_name;

            quote! {
                #[doc = " Test generated by #[catga_service] macro."]
                #[allow(dead_code)]
                async fn doc_test_ #method_name () -> catga_core::CatgaResult<()> {
                    let registry = Self.registry()?;
                    let _mediator = catga_core::Mediator::new(registry);
                    Ok(())
                }
            }
        }).collect();
