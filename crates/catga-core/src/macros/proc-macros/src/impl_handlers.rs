...
pub fn expand_impl_handlers(
    ...
    // Generate dynamic doc comment for registry() listing all handlers by type
    let registry_doc = {
        ...
        let commands: Vec<_> = method_infos
            .iter()
        ...
        if !requests.is_empty() {
            ...
            parts.push("# Handlers".to_string());
            parts.push("## Requests".to_string());
            for (_, m) in &requests {
    ...
    // Generate doctests for Request handlers
    let requests: Vec<_> = method_infos.iter().filter(|(_, m)| m.is_request).collect();
    let doctest_module = if !requests.is_empty() {
        // Generate a compile-time check that registry() exists
        // Uses a const assertion to validate the registry function signature
        quote! {
            #[cfg(test)]
            #[allow(dead_code)]
            mod __catga_service_doctests {
                // Compile-time validation that #[catga_service] generated a registry() function
                const _: () = {
                    fn __assert_registry_exists<T: FnOnce() -> catga_core::CatgaResult<catga_core::Registry>>() {}
                };
            }
        }
    } else {
    ...
    if let Some(mediator_name) = typed_mediator_name {
...
fn extract_response_type_name(output: &syn::ReturnType) -> Option<String> {
    ...
    Some(quote!(#inner_ty).to_string().trim().to_string())
}
