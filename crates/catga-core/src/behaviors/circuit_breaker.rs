...
mod tests {
    ...
    #[test]
    fn circuit_breaker_options_builder_defaults() {
        let builder = CircuitBreakerOptions::builder(5, Duration::from_secs(30));
        let options = builder.build().expect("valid options");

        assert_eq!(options.sampling_window(), 5);
        assert_eq!(options.minimum_throughput(), 5);
        assert_eq!(options.failure_ratio_numerator(), 1);
        assert_eq!(options.failure_ratio_denominator(), 1);
        assert_eq!(options.reset_timeout(), Duration::from_secs(30));
    }

    #[test]
    fn circuit_breaker_options_builder_sampling_window() {
        let builder =
            CircuitBreakerOptions::builder(10, Duration::from_secs(30)).sampling_window(100);
        let options = builder.build().expect("valid options");
        assert_eq!(options.sampling_window(), 100);
    }

    #[test]
    fn circuit_breaker_options_builder_minimum_throughput() {
        let builder =
            CircuitBreakerOptions::builder(10, Duration::from_secs(30)).minimum_throughput(5);
        let options = builder.build().expect("valid options");
        assert_eq!(options.minimum_throughput(), 5);
    }

    #[test]
    fn circuit_breaker_options_builder_failure_ratio() {
        let builder =
            CircuitBreakerOptions::builder(10, Duration::from_secs(30)).failure_ratio(1, 2);
        let options = builder.build().expect("valid options");
        assert_eq!(options.failure_ratio_numerator(), 1);
        assert_eq!(options.failure_ratio_denominator(), 2);
    }

    #[test]
    fn circuit_breaker_options_rejects_zero_failure_threshold() {
        let result = CircuitBreakerOptions::builder(0, Duration::from_secs(30)).build();
        assert!(matches!(result, Err(e) if e.code() == ErrorCode::Validation));
    }

    #[test]
    fn circuit_breaker_options_rejects_zero_reset_timeout() {
        let result = CircuitBreakerOptions::builder(5, Duration::ZERO).build();
        assert!(matches!(result, Err(e) if e.code() == ErrorCode::Validation));
    }

    #[test]
    fn circuit_breaker_options_rejects_zero_sampling_window() {
        let result = CircuitBreakerOptions::builder(5, Duration::from_secs(30))
            .sampling_window(0)
            .build();
        assert!(matches!(result, Err(e) if e.code() == ErrorCode::Validation));
    }
    ...
    #[test]
    fn circuit_breaker_options_rejects_excessive_sampling_window() {
        let result = CircuitBreakerOptions::builder(5, Duration::from_secs(30))
            .sampling_window(10001)
            .build();
        assert!(matches!(result, Err(e) if e.code() == ErrorCode::Validation));
    }

    #[test]
    fn circuit_breaker_options_rejects_zero_minimum_throughput() {
        let result = CircuitBreakerOptions::builder(5, Duration::from_secs(30))
    ...
    #[test]
    fn circuit_breaker_options_rejects_minimum_throughput_exceeding_window() {
        let result = CircuitBreakerOptions::builder(5, Duration::from_secs(30))
            .sampling_window(10)
    ...
    #[test]
    fn circuit_breaker_options_rejects_zero_numerator() {
        let result = CircuitBreakerOptions::builder(5, Duration::from_secs(30))
            .failure_ratio(0, 1)
            .build();
        assert!(matches!(result, Err(e) if e.code() == ErrorCode::Validation));
    ...
    #[test]
    fn circuit_breaker_options_rejects_zero_denominator() {
        let result = CircuitBreakerOptions::builder(5, Duration::from_secs(30))
            .failure_ratio(1, 0)
            .build();
        assert!(matches!(result, Err(e) if e.code() == ErrorCode::Validation));
    ...
    #[test]
    fn circuit_breaker_options_rejects_numerator_exceeding_denominator() {
        let result = CircuitBreakerOptions::builder(5, Duration::from_secs(30))
            .failure_ratio(2, 1)
            .build();
        assert!(matches!(result, Err(e) if e.code() == ErrorCode::Validation));
    ...
    #[test]
    fn circuit_breaker_new_accepts_valid_parameters() {
        let result = CircuitBreakerBehavior::new(5, Duration::from_secs(30));
        assert!(result.is_ok());
    }

    #[test]
    fn circuit_breaker_new_rejects_invalid_parameters() {
        let result = CircuitBreakerBehavior::new(0, Duration::from_secs(30));
        assert!(result.is_err());

        let result = CircuitBreakerBehavior::new(5, Duration::ZERO);
        assert!(result.is_err());
    }

    #[test]
    fn circuit_breaker_with_options_creates_instance() {
        let options = CircuitBreakerOptions::builder(10, Duration::from_secs(30))
            .failure_ratio(1, 2)
            .build()
            .expect("valid options");
        let breaker = CircuitBreakerBehavior::with_options(options);
        // Basic check that instance was created
        assert!(std::mem::size_of_val(&breaker) > 0);
    }

    #[test]
    fn outcome_window_push_and_clear() {
    ...
    #[test]
    fn outcome_window_respects_capacity_limit() {
        let mut window = OutcomeWindow::new(2);
        window.push(true);
        window.push(false);
        window.push(true); // Should evict the first true
        assert_eq!(window.len(), 2);
        assert_eq!(window.failures(), 1); // The false remains
    }

    #[test]
    fn outcome_window_tracks_failures_correctly() {
        ...
        // Add more to fill the window
        window.push(false);
        assert_eq!(window.failures(), 3);

        // This should evict first true, keeping failures at 3
        window.push(true);
        assert_eq!(window.failures(), 3);
    }
}
