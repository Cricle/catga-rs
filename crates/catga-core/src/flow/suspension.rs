...
pub struct FlowChildLaunch {
    pub(crate) child_id: Box<str>,
    pub(crate) state: FlowChildLaunchState,
}
...
pub struct WaitResult {
    pub(crate) child_id: Box<str>,
    #[serde(
        serialize_with = "serialize_optional_arc_slice",
        deserialize_with = "deserialize_optional_arc_slice"
    )]
    pub(crate) payload: Option<Arc<[u8]>>,
    pub(crate) error: Option<CatgaError>,
}
