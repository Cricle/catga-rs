...
mod tests {
    ...
    use crate::{
        ...
        CommandPipeline, ErrorCode, Event, EventHandler, Handler, Mediator, MediatorHandle,
    ...
    fn max_mediator_batch_size_value() {
        assert_eq!(crate::MAX_MEDIATOR_BATCH_SIZE, 1024);
    }
    ...
    async fn send_batch_rejects_oversized_batch() {
        ...
        let oversized: Vec<TestRequest> = (0..=crate::MAX_MEDIATOR_BATCH_SIZE)
            .map(|_| TestRequest)
            .collect();
    ...
    // Test publish_with_concurrency validation
    #[tokio::test]
    ...
    // Test command pipeline validation
    #[tokio::test]
    async fn command_pipeline_depth_limit() {
        ...
        for _ in 0..=crate::MAX_PIPELINE_DEPTH {
    ...
    // Test for EventHandler failure propagation
    #[tokio::test]
    ...
    // Test for concurrent event publishing
    #[tokio::test]
    async fn publish_with_concurrency_runs_handlers_concurrently() {
        ...
        impl EventHandler<TestEvent> for SlowHandler {
            async fn handle(&self, _: TestEvent) -> CatgaResult<()> {
                // Record that this handler started
        ...
        // With concurrency 2, both handlers run in parallel, so should take ~50ms
