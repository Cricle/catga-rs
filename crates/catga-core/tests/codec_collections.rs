...
fn standard_collections_round_trip_empty_and_populated_values() {
    ...
    assert_eq!(round_trip(&VecDeque::from([1_i32, 2])), VecDeque::from([1, 2]));
    assert_eq!(
        round_trip(&LinkedList::from([1_i32, 2])),
        LinkedList::from([1, 2])
    );
    assert_eq!(round_trip(&[1_i32, 2, 3]), [1, 2, 3]);
    assert_eq!(round_trip(&HashSet::from([1_i32, 2])), HashSet::from([1, 2]));
    assert_eq!(
...
fn collection_decoders_reject_negative_and_mismatched_lengths() {
    assert!(
        MemoryPackSerializer::deserialize::<Vec<i32>>(&(-2_i32).to_le_bytes()).is_err()
    );
    assert!(
        MemoryPackSerializer::deserialize::<[i32; 2]>(&1_i32.to_le_bytes()).is_err()
    );
    assert!(
        MemoryPackSerializer::deserialize::<[i32; 2]>(&(-1_i32).to_le_bytes()).is_err()
    );
