//! NATS transport performance tests.
//!
//! These tests require an external NATS server and are ignored by default.
//! Run with: `cargo test -p catga-nats --test performance -- --ignored`
...
async fn client_connection_establishment() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("NATS connection establishment: {:?}", connect_elapsed);
    ...
    while recv_count < COUNT && Instant::now() < deadline {
        match tokio::time::timeout(Duration::from_millis(100), transport.receive()).await {
            Ok(Ok(delivery)) => {
                recv_count += 1;
...
async fn error_handling_under_failure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = NatsConfig {
        server: "nats://127.0.0.1:9999".into(),
        stream: "test".into(),
        subject: "test".into(),
        consumer: "test".into(),
    };

    let result = NatsTransport::connect(config).await;
    match result {
...
async fn batch_publish_throughput() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for batch_size in [1, 10, 100, 1000] {
        ...
        let start = Instant::now();
        for _ in 0..100 {
            transport.publish_batch(envelopes.clone()).await?;
        }
