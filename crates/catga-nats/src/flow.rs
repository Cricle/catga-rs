...
mod tests {
    ...
    #[test]
    fn flow_index_keys_and_cursor_transitions_are_stable_and_bounded() {
        assert_eq!(flow_key("flow-a"), flow_key("flow-a"));
        assert_ne!(flow_key("flow-a"), flow_key("flow-b"));
        assert_ne!(type_metadata_key("orders"), type_page_key("orders", 0));
        assert_ne!(
            type_marker_key("orders", "a"),
            type_marker_key("orders", "b")
        );
        ...
        let consumed = next_index_cursor(&metadata, true).expect("cursor advances within page");
        assert_eq!(consumed.scan_page, 1);
        assert_eq!(consumed.scan_offset, 8);

        let next_page = next_index_cursor(&metadata, false).expect("cursor advances to next page");
        assert_eq!(next_page.scan_page, 2);
        assert_eq!(next_page.scan_offset, 0);

        let wrapped = next_index_cursor(
            &TypeIndex {
                tail_page: 2,
                scan_page: 2,
                scan_offset: 0,
        ...
        .expect("tail page wraps to the first page");
        assert_eq!(wrapped.scan_page, 0);
        ...
        assert_eq!(
            ...
            .expect_err("offset overflow is rejected")
            .code(),
            ErrorCode::Internal
        );
        assert!(validate_index_page(&vec!["id".into(); MAX_INDEX_PAGE_ENTRIES]).is_ok());
        assert_eq!(
            validate_index_page(&vec!["id".into(); MAX_INDEX_PAGE_ENTRIES + 1])
                .expect_err("oversized index pages are rejected")
                .code(),
            ErrorCode::Internal
        );
    }
    ...
    #[test]
    fn flow_state_frames_are_bounded_and_staleness_is_monotonic() {
        let state = FlowState::new("flow-1", "orders", [1_u8, 2, 3], "worker-a");
        let bytes = encode(&state).expect("flow state serializes");
        assert_eq!(
            decode_flow_state(&bytes).expect("flow state deserializes"),
            state
        );
        assert_eq!(
            decode_flow_state(&[0xff])
                .expect_err("malformed flow state is rejected")
                .code(),
            ErrorCode::Internal
        );
        ...
        assert_eq!(cas_error("update").code(), ErrorCode::Transient);
