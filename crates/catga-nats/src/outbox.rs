...
mod tests {
    ...
    #[test]
    fn only_wrong_last_sequence_is_a_claim_revision_conflict() {
        assert!(is_revision_conflict_kind(
        ...
        assert!(!is_revision_conflict_kind(
            jetstream::context::PublishErrorKind::Other
        ));
    }
    ...
    fn legacy_record_decodes_with_the_default_retry_policy() -> CatgaResult<()> {
        ...
        let owner = b"worker-a";
        let owner_length = u16::try_from(owner.len())
            .map_err(|_| CatgaError::new(ErrorCode::Internal, "test outbox owner is too long"))?;
        let mut legacy = Vec::with_capacity(2 + owner.len() + payload.len());
        legacy.extend_from_slice(&owner_length.to_be_bytes());
        legacy.extend_from_slice(owner);
        legacy.extend_from_slice(&payload);

        let record = decode(&codec, &legacy)?;
        assert_eq!(record.owner.as_ref(), "worker-a");
        assert_eq!(record.state, StoredState::Claimed);
        assert_eq!(record.message.state(), catga_core::OutboxState::Claimed);
        assert_eq!(record.message.owner(), Some("worker-a"));
        assert_eq!(record.message.retry_count(), 0);
        assert_eq!(record.message.max_retries(), DEFAULT_OUTBOX_MAX_RETRIES);
        Ok(())
    }
    ...
    fn current_record_round_trips_published_state_and_timestamp() -> CatgaResult<()> {
        ...
        let mut message = OutboxMessage::new(Envelope::new(
            ...
            "order.published",
            vec![2],
            MessageMetadata::new(8, None),
        ));
        message.claim("worker-a");
        message.mark_published(123_456);

        let encoded = encode(&codec, StoredState::Published, &message)?;
        let decoded = decode(&codec, &encoded)?;
        assert_eq!(decoded.state, StoredState::Published);
        assert_eq!(decoded.message.state(), catga_core::OutboxState::Published);
        assert_eq!(decoded.message.published_at_unix_ms(), Some(123_456));
        Ok(())
    }
}
