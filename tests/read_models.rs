...
async fn synchronizer_only_marks_changes_after_the_strategy_succeeds() {
    ...
    assert!(tracker.pending_page(1).await.unwrap().is_empty());
    ...
    ReadModelSynchronizer::new(&tracker, &failing)
        ...
        .await
        .unwrap_err();
    assert_eq!(tracker.pending_page(1).await.unwrap().len(), 1);
}

#[tokio::test]
async fn synchronizer_processes_one_bounded_pending_page_per_run() {
    let tracker = MemoryChangeTracker::default();
    for id in 0..3 {
        tracker.track(ChangeRecord::new(
            id.to_string(),
    ...
    let synchronizer =
        ReadModelSynchronizer::with_page_size(&tracker, &strategy, NonZeroUsize::new(2).unwrap());

    synchronizer.sync().await.unwrap();

    assert_eq!(deliveries.load(Ordering::Relaxed), 2);
    assert_eq!(tracker.pending_page(3).await.unwrap().len(), 1);
    synchronizer.sync().await.unwrap();
